...
env_setup_vm(struct Env *e)
{
    ...
    //	      above UTOP.
    //	(Make sure you got the permissions right in Ashish 2.)
	...
	e->env_pml4e[0] = (page2pa(pdpe_env_page)) | PTE_P | PTE_U | PTE_W;
	...
	pdpe_va[3] = (page2pa(pgdir_env_page)) | PTE_P | PTE_U | PTE_W;

	pdpe_t *pdpe_boot = KADDR(PTE_ADDR(boot_pml4e[0]));
	pde_t  *pgdir_boot = KADDR(PTE_ADDR(pdpe_boot[3]));
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
    ...
    //  Use functions from the previous lll to allocate and map pages.
    //
