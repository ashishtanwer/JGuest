...
boot_alloc(uint32_t n)
{
    ...
    if (!nextfree) {
#ifdef VMM_GUEST //Ashish: To check defined
        extern char end[];
...
x64_vm_init(void)
{
    ...
    // Permissions: kernel RW, user NONE
	boot_map_segment(boot_pml4e, KERNBASE, 0x100000000 - KERNBASE, 0x0, PTE_W | PTE_P);
    ...
    //////////////////////////////////////////////////////////////////////
    // Permissions: kernel RW, user NONE
    //Ashish: To check the permissions 
...
page_init(void)
{
    ...
    for (i = 0; i < npages; i++) {
		...
		// Ashish : To check the contants
			...
			//Ashish doubt 0xF0100000  ==> 0x8004000000
			((int*)page2kva(&pages[i]) >= (int*)(0x8004100000) &&
...
{
    //Ashish: check the paging levels
...
check_boot_pml4e(pml4e_t *pml4e)
{
    ...
    // check envs array (new test for Ashish 3)
    ...
    // check kernel stack
	//Ashish
    // (updated in Ashish 4 to check per-CPU kernel stacks)
    for (n = 0; n < NCPU; n++) {
